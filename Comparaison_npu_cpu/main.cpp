...
int main(int argc, char ** argv) {
    ...
    save_vocab_to_file(vocab, "llama_vocab.txt");
    ...
    // print system information system_info: n_threads = 8 (n_threads_batch = 8) / 8 | CPU : NEON = 1 | ARM_FMA = 1 | FP16_VA = 1 | DOTPROD = 1 | LLAMAFILE = 1 | OPENMP = 1 | AARCH64_REPACK = 1 |
    {
        LOG_INF("\n");
...

    {
        auto prompt = (params.conversation && params.enable_chat_template && !params.prompt.empty())
            ...
            : params.prompt;
        LOG_INF("params.conversation=%d, params.enable_chat_template=%d, params.prompt.empty()=%d\n",
        params.conversation, params.enable_chat_template, params.prompt.empty());
        ...
        if (params.interactive_first || !params.prompt.empty() || session_tokens.empty()) {
            ...
            LOG_INF("Params: interactive_first=%d, prompt=%s, session_tokens.size()=%zu\n", 
        params.interactive_first, 
        params.prompt.empty() ? "(empty)" : params.prompt.c_str(), 
        session_tokens.size());
    ...
    if (!embd_inp.empty() && n_matching_session_tokens == embd_inp.size() && session_tokens.size() > embd_inp.size()) {
        LOG_INF("recalculate the cached logits (do): session_tokens.resize( %zu )\n", embd_inp.size() - 1);
