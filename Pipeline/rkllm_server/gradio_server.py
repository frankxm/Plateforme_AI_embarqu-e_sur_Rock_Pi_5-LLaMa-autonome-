...
class RKLLMParam_ori(ctypes.Structure):
    _fields_ = [
...
def callback_impl_ori(result, userdata, state):
...
class RKLLM(object):
    def __init__(self, model_path, lora_model_path = None, prompt_cache_path = None):
        ...
        rkllm_param.max_context_len =128 
        rkllm_param.max_new_tokens = 192
...
#        self.rkllm_run.argtypes = [RKLLM_Handle_t, ctypes.POINTER(RKLLMInput), ctypes.POINTER(RKLLMInferParam), ctypes.c_void_p]
    ...
    def run_ori(self, prompt):
        ...
        print(f"Prompt: {PROMPT_TEXT_PREFIX + prompt + PROMPT_TEXT_POSTFIX}")
        print(f"RKLLMInput: {rkllm_input}")
        print(f"RKLLMInferParam: {rkllm_infer_params}")
    ...
    def run(self, prompt):
        ...
        result = self.rkllm_run(self.handle, formatted_prompt_c, None)
        print(f"rkllm_run returned: {result}")
...
if __name__ == "__main__":
    ...
    def get_RKLLM_output_ori(history):
    ...
    with gr.Blocks(title="Chat with RKLLM") as chatRKLLM:
        ...
        rkllmServer = gr.Chatbot(height=600, type='messages')
